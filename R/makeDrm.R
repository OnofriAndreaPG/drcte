makeDrm <- function(counts, treat, nViable, moniTimes) {
  ...
  datiFin <- data.frame(group, Unit = Dish, timeBef = as.numeric(timeBef),
                        timeAf = as.numeric(timeAf), count = nSeeds, nCum = nCum, propCum = Prop)
  row.names(datiFin) <- 1:length(datiFin[,1])
  return(datiFin)
}
